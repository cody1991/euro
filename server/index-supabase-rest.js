...
app.get('/api/budgets', async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('budgets')
      .select('*')
      .eq('itinerary_id', req.query.itinerary_id);
...
// 获取预算分类
app.get('/api/budget_categories', async (req, res) => {
  try {
    const { data, error } = await supabase
...
// 创建预算分类
app.post('/api/budget_categories', async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('budget_categories')
      .insert([req.body])
...
// 更新预算分类
app.put('/api/budget_categories', async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('budget_categories')
      .update(req.body)
...
// 删除预算分类
app.delete('/api/budget_categories', async (req, res) => {
  try {
    const { error } = await supabase
      .from('budget_categories')
      .delete()
...
app.get('/api/expenses', async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('expenses')
      .select('*')
      .eq('budget_id', req.query.budget_id);
